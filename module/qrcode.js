...
var ECC_CODEWORDS_PER_BLOCK = [
	[
		0, 7, 10, 15, 20, 26, 18, 20, 24, 30, 18, 20, 24, 26, 30, 22, 24, 28, 30, 28, 28, 28, 28, 30, 30, 26, 28, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30,
		30, 30,
	],
	[
		0, 10, 16, 26, 18, 24, 16, 18, 22, 22, 26, 30, 22, 22, 24, 24, 28, 28, 26, 26, 26, 26, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28,
		28, 28,
	],
	[
		0, 13, 22, 18, 26, 18, 24, 18, 22, 20, 24, 28, 26, 24, 20, 30, 24, 28, 28, 26, 30, 28, 30, 30, 30, 30, 28, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30,
		30, 30,
	],
	[
		0, 17, 28, 22, 16, 22, 28, 26, 26, 24, 28, 24, 28, 22, 24, 24, 30, 28, 28, 26, 28, 30, 24, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30,
		30, 30,
	],
];
var ECC_BLOCKS = [
...
class BitBuffer {
	...
	toBytes() {
		...
		this.bits.forEach((bit, index) => {
			if (bit === 1) bytes[index >>> 3] = bytes[index >>> 3] | (128 >>> (index % 8));
		});
		return bytes;
	}
}
...
class QRCode {
	...
	drawFormatBits(mask) {
		...
		let remainder = data;
		for (let i = 0; i < 10; i++) remainder = (remainder << 1) ^ ((remainder >>> 9) * 1335);
	...
	drawVersionBits() {
		...
		for (let i = 0; i < 12; i++) remainder = (remainder << 1) ^ ((remainder >>> 11) * 7973);
...
function buildCodewords(segments, version, level) {
	...
	for (let column = 0; column < longestBlock; column++) {
		for (const block of dataBlocks) {
			if (column < block.length) result[index++] = block[column];
		}
	}
	for (let column = 0; column < eccPerBlock; column++) {
		for (const block of eccBlocks) result[index++] = block[column];
	}
	return result;
}
...
export { ErrorCorrectionLevel, MAX_VERSION, MIN_VERSION, Mode, QRCode, toDataURL, toSVG, toText };
