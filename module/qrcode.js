  ...
  toSVG(options = {}) {
    ...
    for (let y = this.size - 1;y >= 0; y--) {
      ...
      for (let x = 0;x <= this.size; x++) {
        ...
        if (parts.length === 0)
          parts.push(`M${column} ${y + shift}.5h${width}`);
        else
          parts.push(`m${column - penX}-${penY - y}h${width}`);
        penX = column + width;
        penY = y;
        start = -1;
      }
    ...
    return `${declaration}<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${extent} ${extent}"${dimensions} ` + `shape-rendering="crispEdges" role="img">${title}${frame}${background}` + `<path stroke="${escapeXml(dark)}"${transform} d="${parts.join("")}"/>${logo}</svg>`;
  }
